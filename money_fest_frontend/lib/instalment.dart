...
void _showSetReminderPopup(BuildContext context, int index) {
  ...
  List<DateTime> reminders = List<DateTime>.from(category['reminders'] ?? []); // Mendapatkan daftar pengingat
  ...
  onPressed: () {
    if (instalmentDeadline == null) {
      ...
      setState(() {
        _categories.add({
          'name': notes ?? 'No notes',
          'assigned': instalmentDeadline != null ? DateFormat('yyyy-MM-dd').format(instalmentDeadline!) : 'No deadline',
